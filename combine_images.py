...
def combine_images(output_dir='static'):
    ...
    if os.path.exists(combined_image_path):
        # Remove the old image before saving the new one to ensure no caching issues
        os.remove(combined_image_path)
    combined_image.save(combined_image_path)
    print(f"Images combined successfully into '{combined_image_path}'")

    # If also want to copy to static for legacy reasons, uncomment below:
    # import shutil
    # shutil.copy2(combined_image_path, os.path.join('static', 'combined_questions.jpg'))
